...
- PDF 优先本地文本层解析。
- PDF 页面渲染 DPI 可配置，默认 `PDF_RENDER_DPI=220`。
- 文本层不可靠时生成页面截图，再调用千问视觉 OCR。
- OCR 已抽成 provider 入口，当前默认 `OCR_PROVIDER=qwen`，预留 `mathpix`。
- 版面坐标识别已抽成 `recognizeLayout` provider，当前默认 `OCR_LAYOUT_PROVIDER=qwen`。
- OCR 前支持 `sharp` 图片预处理：自动旋转、灰度、normalize、sharpen，可用 `OCR_PREPROCESS=false` 关闭。
- 支持多页后台分析任务和进度轮询。
- 上传记录会保存 `analysisDiagnostics`，可追踪每页 OCR、候选题、AI 返回、去重、跳过和失败原因。
...
- 待审核题保存 `questionBBox`，裁图优先使用坐标；手动重新框选会保存真实原页坐标。
- 待审核题支持人工合并上/下题、按分隔线拆分为多题。
- 待审核题支持 revision history，记录人工编辑、AI 补全、配图更新、合并、拆分。
...
- 知识点快捷标签可快速筛选。
- 题库页新增知识点树，按科目、章节/年级、知识点聚合；点击知识点可直接筛选题库。
- 题库题目可编辑、AI 分类、删除。
...
- 作业页面可预览。
- 支持导出 Word `.docx`。
- 导出版本支持学生版、答案版、解析版；后端 Word 导出接口同步支持 `exportMode`。
- 题目配图会一起导出到 Word。
...
- 支持学生档案。
- 支持记录错题原因、备注。
- 学生页新增薄弱知识点概览，会根据错题记录统计每个学生最薄弱知识点和主要错误原因。
...
- 设置页新增上线准备清单，展示 AI Key/用量、题库数量、学生错题、作业、多人账号、上传存储等产品化状态。
...
- 待审核单题工作台改成三块结构：左侧原页参考，中间本题截图/粘贴/框选，右侧结构化字段。
- 待审核区新增“通过低风险题”，只批量通过没有质检错误和警告的题。
- 题库页新增统计卡和知识点快捷筛选。
- 作业生成新增学生版、答案版、解析版导出选择。
- 学生页新增薄弱知识点概览。
- 设置页新增产品上线准备清单。
...
- 待审核题会自动生成候选截图并随题入库；当前优先按 `questionBBox` 裁剪，`questionBBox` 可来自手动框选、千问视觉 layout、AI bbox 或文本行位置估算。
...
2. 继续优化 PDF 题目配图：
   - 引入 OCR 坐标/版面块后，直接生成真实题目 bbox，提高自动裁剪准确率。
   - 对“如图”题单独识别题内图形/表格区域。
